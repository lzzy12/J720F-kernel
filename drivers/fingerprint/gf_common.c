			...
			unsigned long event, void *data)
{
	...
	case FB_BLANK_UNBLANK:
		pr_info("%s : lcd on notify\n", __func__);
		gfspi_netlink_send(gf_dev, GF_NETLINK_SCREEN_ON);
		break;

	case FB_BLANK_POWERDOWN:
		pr_info("%s : lcd off notify\n", __func__);
		gfspi_netlink_send(gf_dev, GF_NETLINK_SCREEN_OFF);
		break;

	default:
		pr_info("%s : other notifier, ignore\n", __func__);
		break;
	}
	return retval;
}
...
static unsigned int gfspi_poll(struct file *filp, struct poll_table_struct *wait)
{
...
static int gfspi_set_timer(struct gf_device *gf_dev)
{
	...
	setup_timer(&gf_dev->dbg_timer,	gfspi_timer_func, (unsigned long)gf_dev);
...
void gfspi_hw_power_enable(struct gf_device *gf_dev, u8 onoff)
{
	if (onoff && !gf_dev->ldo_onoff) {
		...
		pr_err("%s power is already %s\n",
				__func__, (gf_dev->ldo_onoff ? "Enabled" : "Disabled"));
	} else {
...
int gfspi_type_check(struct gf_device *gf_dev)
{
	...
	chipid32 = (chipid[2] << 24 | chipid[3] << 16 |
			chipid[0] << 8 | chipid[1]);
	if (GF_GW32_CHIP_ID == chipid32) {
		gf_dev->sensortype = SENSOR_GW32J;
		pr_info("%s sensor type is %s\n", __func__,
		sensor_status[gf_dev->sensortype + 2]);
		status = 0;
	} else {
		gf_dev->sensortype = SENSOR_FAILED;
		pr_err("%s sensor type is FAILED 0x%8x\n",
				__func__, chipid32);
	}
	gfspi_hw_power_enable(gf_dev, 0);
	return status;
