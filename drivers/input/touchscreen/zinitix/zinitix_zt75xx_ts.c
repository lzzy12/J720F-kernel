...
static bool init_touch(struct bt532_ts_info *info)
{
	...
	if (!mini_init_touch(info))
		goto fail_init;
#ifdef TCLM_CONCEPT
	get_tsp_nvm_data(info, BT532_TS_NVM_OFFSET_FAC_RESULT, (u8 *)data, 2);
...
#if defined(CONFIG_TOUCHSCREEN_DUMP_MODE)
	if (!info->check_rawdata_done) {
		input_err(true, &info->client->dev, "%s not finished checking rawdata\n", __func__);
		return 0;
	}
#endif
...
static void get_tsp_test_result(void *device_data)
{
	...
	get_tsp_nvm_data(info, BT532_TS_NVM_OFFSET_FAC_RESULT, (u8 *)buff, 2);
...
static void set_tsp_test_result(void *device_data)
{
	...
	set_tsp_nvm_data(info, BT532_TS_NVM_OFFSET_FAC_RESULT, &info->test_result.data[0], 1);
...
static void increase_disassemble_count(void *device_data)
{
	...
	get_tsp_nvm_data(info, BT532_TS_NVM_OFFSET_DISASSEMBLE_COUNT, count, 2);
	...
	set_tsp_nvm_data(info, BT532_TS_NVM_OFFSET_DISASSEMBLE_COUNT, count , 2);
...
int bt532_tclm_data_read(struct i2c_client *client, int address)
{
	...
	case SEC_TCLM_NVM_OFFSET_HISTORY_QUEUE_SIZE:
		...
		for (i = 0; i < info->tdata->cal_pos_hist_cnt; i++) {
			get_tsp_nvm_data(info, BT532_TS_NVM_OFFSET_HISTORY_QUEUE_ZERO + 2 * i,
	...
	if (tune_ver_flag) {
		get_tsp_nvm_data(info, reg, (u8 *)buff, 2);
		ret = (buff[1] << 8) | buff[0];
	} else {
		get_tsp_nvm_data(info, reg, (u8 *)buff, 2);
		ret = buff[0];
...
void bt532_tclm_data_write(struct i2c_client *client, int address, int data)
{
	...
	case SEC_TCLM_NVM_OFFSET_HISTORY_QUEUE_SAVE:
		...
		set_tsp_nvm_data(info, BT532_TS_NVM_OFFSET_HISTORY_QUEUE_ZERO + info->tdata->cal_pos_hist_lastp * 2, (u8*)buff, 2);
		return;
	default:
		return;
	}
	...
	if (tune_ver_flag) {
		buff[0] = data & 0xff;
		buff[1] = (data >> 8) & 0xff;
		set_tsp_nvm_data(info, reg, (u8*)buff, 2);
	} else {
		buff[0] = data;
		buff[1] = 0;
		set_tsp_nvm_data(info, reg, (u8*)buff, 2);
...
{
	struct bt532_ts_info *info = container_of(work, struct bt532_ts_info, ghost_check.work);
