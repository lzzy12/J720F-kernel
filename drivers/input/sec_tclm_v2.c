...
int sec_tclm_get_nvm_all(struct sec_tclm_data *data)
{
	...
	if ((data->nvdata.cal_pos_hist_cnt < 0) || (data->nvdata.cal_pos_hist_cnt > CAL_HISTORY_QUEUE_MAX))
...
int sec_execute_tclm_package(struct sec_tclm_data *data, int factory_mode)
{
	...
	while (retry) {
		...
		if (ret < 0) {
			input_err(true, &data->client->dev, "%s: SEC_TCLM_NVM_ALL_DATA i2c write fail", __func__);
			if (!retry--)
				goto out;
		} else {
			break;
